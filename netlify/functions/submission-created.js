...
exports.handler = async (event) => {
  ...
  if (!form_name || !form_name.startsWith(FORM_NAME_PREFIX)) {
    console.log(`Formulario "${form_name}" no coincide con el prefijo "${FORM_NAME_PREFIX}". Saltando.`);
    return {
      statusCode: 200,
  ...
  if (botField) {
    console.warn("Se detectó un bot (honeypot lleno).");
    return { statusCode: 400, body: JSON.stringify({ error: "Spam submission detected." }) };
  }
  if (!name || !comment || !article_slug || !article_title) {
    const missing = [!name && "name", !comment && "comment", !article_slug && "article_slug", !article_title && "article_title"]
      .filter(Boolean)
      .join(", ");
    console.error("Campos requeridos faltantes:", missing);
    return { statusCode: 400, body: JSON.stringify({ error: `Missing required fields: ${missing}` }) };
  }
  if (comment.length > 2000) {
    console.error("El comentario excede el límite permitido");
  ...
  console.log("Nombre de la nueva rama:", newBranchName);

  try {
    // Importa Octokit dinámicamente
    const { Octokit } = await import("@octokit/rest");
    const octokit = new Octokit({ auth: GITHUB_TOKEN });
    console.log("Octokit importado e inicializado correctamente.");
    ...
    console.log("SHA de la rama main:", mainBranchSha);

    // Crear la nueva rama a partir de main
    ...
    // Intentar obtener el contenido actual del archivo en la nueva rama
    ...
    try {
      ...
      if (fileData.content) {
        ...
        console.log("Comentarios existentes cargados desde el archivo.");
      }
    } catch (error) {
      if (error.status !== 404) {
      ...
      console.log(`El archivo "${commentsFilePath}" no existe en la rama "${newBranchName}". Se creará uno nuevo.`);
    }
    ...
    // Crear o actualizar el archivo de comentarios en la nueva rama
    ...
    console.log(`Archivo "${commentsFilePath}" actualizado en la rama "${newBranchName}".`);

    // Crear el Pull Request desde la rama nueva a la rama main
...
This pull request will merge branch \`${newBranchName}\` into \`${REPO_BRANCH}\`.
    `;
    ...
    console.log(`Pull request creado: ${pullRequest.html_url}`);
    ...
    if (error.response && error.response.data) console.error("GitHub API Error:", error.response.data);

    // Intento opcional de limpiar la rama creada si ocurrió un error
    try {
      const { Octokit } = await import("@octokit/rest");
      const octokit = new Octokit({ auth: GITHUB_TOKEN });
